...
object W1_warmup {
  ...
  def indexOfMax(a : Array[Int]) : Int = {
    ...
    if (a.length > 0){
      ...
      for (i <- a.indices.tail) { // goes i = 1 2 3 ... until end of array //doing a.tail.indices it goes 0 1 2 3 until end of array - 1 so cuts of the other end
        if (a(i) > maxNum){
          maxIndex = i
          maxNum = a(i)
        }
...
Indication of length : 12 lines
  ...
  def diamondString(height : Int) : String = {
    var diamond = ""
    val halfway = (height + 1) / 2 // So that the spaces align
...
Indication of length : 7 lines
  ...
  def wordCount(text : String) : mutable.Map[String,Int] = {
    val list = text.toLowerCase.split("(\\s|\\.|,|!)+")
    val count: mutable.Map[String, Int] = mutable.Map()

    for (i <- list.indices) {
      if (count.contains(list(i))) {
        count.update(list(i), count(list(i)) + 1) // word appears once more, +1
      }
      else {
        count += (list(i) -> 1) // add new word to map
      }
    }
    return count
  }
...
Indication of length: 25 added lines. Add extra functions to organize your code.
...
Indication of length: 12 lines
  ...
  def splitArray(a : Array[Int]) : (Array[Int],Array[Int]) = {
    val lhsLength = a.length / 2
    val rhsLength = a.length - lhsLength

    val lhs = new Array[Int](lhsLength)
    val rhs = new Array[Int](rhsLength)

    for (i <- 0 until  lhsLength) {
      lhs(i) = a(i)
    }

    for (i <- 0 until rhsLength) {
      rhs(i) = a(lhsLength + i)
    }
...
Indication of length: 15 lines
  ...
  def mergeSortedArrays(a : Array[Int], b : Array[Int]) : Array[Int] = {
    val totalIndices = a.length + b.length
    var aIndex = 0
    var bIndex = 0

    val sortedArray = new Array[Int](totalIndices)

    for (i <- 0 until  totalIndices) {
      if (aIndex < a.length && (bIndex == b.length || a(aIndex) <= b(bIndex))){
        sortedArray(i) = a(aIndex)
        aIndex += 1
      }
      else {
        sortedArray(i) = b(bIndex)
        bIndex += 1
      }
    }
    return sortedArray
  ...
  More precisely, mergeSort works as follows:
   ...
   If the input array is of length <= 1, then the array is already sorted and the input array is returned.
...
Indication of length: 5 lines
  ...
  def mergeSort(a : Array[Int]) : Array[Int] = {
    ...
    val (lhs, rhs) = splitArray(a)

    val sortedLhs = mergeSort(lhs) //recursion: L L L L Return(<=1) R Return(sorted) R Return(sorted) R Return(sorted) R Return(sorted)
    val sortedRhs = mergeSort(rhs)

    return mergeSortedArrays(sortedLhs, sortedRhs)
  }
